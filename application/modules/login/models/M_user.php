...
class m_user extends CI_Model{
	...
	function update_profile($member_id){
		...
		/*$dob = strtotime($this->input->post('dobDate').' '.$this->input->post('dobMonth').' '.$this->input->post('dobYear'));
		$date_of_birth = date("Y-m-d",$dob);
		$profile_image = $this->uploadImageObject($_FILES['profile_image']);
		if($profile_image){
			$data1 = array('profile_image' => $profile_image);
		}*/
		$org_password = $this->input->post('pwd');
		$pwd = SHA1($org_password);
		$firstname = $this->input->post('firstname');
		if($this->input->post('pwd')){
			$data2 = array('password' => $pwd,'org_password' => $org_password);
		}
		$data3 = array(
			'firstname' => $firstname,
	...
	function isEmailExist($email,$member_id){
		$this->db->select('*');
	...
	function checkpass($pass)
	{
		...
		$query=$this->db->query("select * from tbl_members where id=$member_id and org_password='$pass'");
	...
	//retrieve current loggged user info

	

	function user_infomation($id=0){
		$this->db->select('*');
		$this->db->from('tbl_members');
		if(!empty($id)){
			$this->db->where('id',$id);
